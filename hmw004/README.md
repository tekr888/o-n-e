...
20. Настроить DHCP в офисе Москва, настроить синхронизацию времени в офисе Москва, настроить NAT в офисе Москва, C.-Перетбруг и Чокурдах  
...
- Настройка NAT(PAT) для R14 + удаленный доступ для R19:  
```
!
ip nat inside source list 1 interface Ethernet0/2 overload
ip nat inside source static 100.0.12.19 109.0.13.14
!
access-list 1 permit 100.0.10.0 0.0.0.15
!
```  
На все интрефейсах кроме выходного в сторону AS101 указываем:  
```
ip nat inside
```  
А на Ethernet 0/2:  
...
- Настройка NAT(PAT) для R15 + статический доступ для R20:  
```
!
ip nat inside source list 1 interface Ethernet0/2 overload
ip nat inside source static 100.0.16.20 100.0.13.15
!
access-list 1 permit 10.0.10.0 0.0.0.15
!
```  
На все интрефейсах кроме выходного в сторону AS101 указываем:  
```
...
```
!
ip nat pool NAT_POOL98 98.0.98.18 98.0.98.22 netmask 255.255.255.0
ip nat inside source list 1 pool NAT_POOL98 overload
!
access-list 1 permit 86.86.86.0 0.0.0.31
access-list 1 permit 85.85.85.0 0.0.0.31
!
```  
На интерфейсах в сторону R16 и R17 указываем:  
```
ip nat inside
```  
На интерфейсе в сторону R24 указываем:  
```
ip nat outside
```  
- *. Настроите статический NAT(PAT) для офиса Чокурдах. Пример:  
```
!
ip nat inside source list 1 interface Ethernet0/0 overload
ip nat inside source static tcp 10.177.19.11 80 92.0.92.28 8080 extendable
!
access-list 1 permit 10.177.18.0 0.0.0.255
!
```  
На интерфейсе в сторону SW29 указываем:  
```
ip nat inside
```  
На интерфейсах в сторону R26 и R25 указываем:
```
ip nat ouside
```  
